...
export class CampaignService {
  ...
  public async getCampaignById(
    ...
    if (!campaignId || campaignId.trim().length === 0) {
      throw new BadRequestException("campaignId is required and cannot be empty");
    }
  ...
  public async getCampaignsByOptions(
    ...
    if (!userId || userId.trim().length === 0) {
      this.logger.error("getCampaignsByOptions called without valid userId");
      throw new BadRequestException("userId is required and cannot be empty");
    }
  ...
  public async getTasksByUserWithOptions(
    ...
    if (!userId || userId.trim().length === 0) {
      this.logger.error("getTasksByUserWithOptions called without valid userId");
      throw new BadRequestException("userId is required and cannot be empty");
    }
